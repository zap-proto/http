...
func readFrameInto(r *bufio.Reader, buf []byte) ([]byte, error) {
	...
	if n == 0 {
		return buf, fmt.Errorf("http: zero-length frame")
	}
	if n > MaxFrameSize {
		return buf, fmt.Errorf("http: frame size %d exceeds MaxFrameSize=%d", n, MaxFrameSize)
	}
	if uint32(cap(buf)) < n {
		buf = make([]byte, n)
	} else {
	...
	if _, err := io.ReadFull(r, buf); err != nil {
		return buf, fmt.Errorf("http: short frame: %w", err)
	}
	return buf, nil
}
...
func writeFramePrefixed(w io.Writer, framePlusPrefix []byte) error {
	...
	if body <= 0 {
		return fmt.Errorf("http: refusing to write zero-length frame")
	}
	if uint64(body) > MaxFrameSize {
		return fmt.Errorf("http: frame size %d exceeds MaxFrameSize=%d", body, MaxFrameSize)
	}
...
func writeFrame(w io.Writer, frame []byte) error {
	...
	if uint64(len(frame)) > MaxFrameSize {
		return fmt.Errorf("http: frame size %d exceeds MaxFrameSize=%d", len(frame), MaxFrameSize)
	}
	var hdr [4]byte
